...
// Create 创建数据
func (s *Model) Create(dtoCreate *entity.UserGroup) (*entity.UserGroup, error) {
...
// FindOne 查询一条数据
func (s *Model) FindOne(wheres []*where.Condition) (*entity.UserGroup, error) {
	entityInfo := new(entity.UserGroup)
...
func (s *Model) FindOneById(id uint) (*entity.UserGroup, error) {
	return s.FindOne(where.New("id", "=", id).Conditions())
}

// FindList 查询列表数据
func (s *Model) FindList(wheres []*where.Condition, extra *where.Extra) ([]*entity.UserGroup, uint, error) {

	var entityList []*entity.UserGroup
	...
	//开始统计总数
	total, err := dbops.GetTotal(wheres, new(entity.UserGroup), s.dbRead)
	if err != nil {
		return []*entity.UserGroup{}, 0, err
	}
...
// Update 更新数据
func (s *Model) Update(wheres []*where.Condition, dtoUpdate *entity.UserGroup, columnsCfg ...string) error {
	// dbops.UpdateWithDb(wheres, new(entity.UserGroup), dtoUpdate, s.dbWrite, columnsCfg...)
	err := dbops.Update(&dbops.UpdateConfNew{
		Wheres:           wheres,
		NewEntity:        new(entity.UserGroup),
		Db:               s.dbWrite,
